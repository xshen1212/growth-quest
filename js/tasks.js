...
(function () {
    ...
    function renderCalendar() {
        ...
        const taskHistory = Store.get('task_history') || {};
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            const dayData = taskHistory[dateStr];
            const hasData = dayData && (dayData.daily.length > 0 || dayData.random.length > 0);
            const allComplete = dayData && dayData.allComplete;

            let classes = 'calendar-day';
            if (isToday) classes += ' today';
            if (hasData) classes += ' has-data';
            if (allComplete) classes += ' all-complete';

            html += `<div class="${classes}" data-date="${dateStr}" onclick="showDayDetail('${dateStr}')">${d}</div>`;
        }
    ...
    window.showDayDetail = function (dateStr) {
        ...
        if (!dayData || (dayData.daily.length === 0 && dayData.random.length === 0)) {
            detail.innerHTML = `<div class="gq-card"><div class="gq-empty"><div class="gq-empty-icon">📭</div><div>${Utils.formatDate(dateStr)} 没有任务记录</div></div></div>`;
            return;
        }

        const allDailyTasks = Store.get('daily_tasks') || [];
        const allRandomTasks = Store.get('random_tasks') || [];

        let html = `<div class="gq-card"><div class="gq-card-title">📅 ${Utils.formatDate(dateStr)} 记录</div>`;
        if (dayData.daily.length > 0) {
            html += '<div style="font-weight:600;margin:8px 0 4px">日常任务：</div>';
            dayData.daily.forEach(d => {
                const t = allDailyTasks.find(x => x.id === d.taskId);
                html += `<div style="padding:4px 0;display:flex;gap:8px;align-items:center">
                    <span>✅</span><span>${t ? t.name : d.taskId}</span>
                    <span style="color:var(--text-secondary);font-size:0.8rem">${d.time}</span>
                </div>`;
            });
        }
        if (dayData.random.length > 0) {
            html += '<div style="font-weight:600;margin:8px 0 4px">随机任务：</div>';
            dayData.random.forEach(d => {
                const t = allRandomTasks.find(x => x.id === d.taskId);
                html += `<div style="padding:4px 0;display:flex;gap:8px;align-items:center">
                    <span>🎲</span><span>${t ? t.name : d.taskId}</span>
                    <span style="color:var(--text-secondary);font-size:0.8rem">${d.time}</span>
                </div>`;
        ...
        html += `<div style="margin-top:8px;font-weight:700;color:var(--color-accent-dark)">
            总积分：+${dayData.totalPoints} ${dayData.allComplete ? '🎉 全勤' : ''}</div></div>`;
