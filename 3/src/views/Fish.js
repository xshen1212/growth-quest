...
{
    var captureRate = this.captureRate;
    try {
        var fishConfig = JSON.parse(localStorage.getItem('gq_fish_config'));
        if (fishConfig && Array.isArray(fishConfig)) {
            var fishTypeIndex = 0;
            for (var i = 1; i < ns.R.fishTypes.length; i++) {
                if (ns.R.fishTypes[i] === this.type) {
                    fishTypeIndex = i;
                    break;
                }
            }
            var configFish = fishConfig.find(function(f) { return f.id === fishTypeIndex; });
            if (configFish && configFish.captureRate !== undefined) {
                captureRate = configFish.captureRate;
            }
        }
    } catch(e) {
        console.error('读取鱼配置失败', e);
    }
    return captureRate * (1 + level*0.05) > Math.random();
};
